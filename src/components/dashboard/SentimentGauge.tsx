...
const GaugeChart = memo<{ value: number; label: string }>(({ value, label }) => {
  ...
  useEffect(() => {
    ...
    GAUGE_CONFIG.zones.forEach(zone => {
      const startAngle = minAngle + ((zone.min - minValue) / (maxValue - minValue)) * (maxAngle - minAngle);
      const endAngle = minAngle + ((zone.max - minValue) / (maxValue - minValue)) * (maxAngle - minAngle);
    ...
    // Draw needle
    const needleAngle = minAngle + ((value - minValue) / (maxValue - minValue)) * (maxAngle - minAngle);
...
export const SentimentGauge: React.FC = memo(() => {
  ...
  const currentZone = useMemo(() => {
    if (!sentiment) {return GAUGE_CONFIG.zones[2];} // Default to neutral

    return GAUGE_CONFIG.zones.find(
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-semibold">
                  {sentiment.mentionlyticsScore.toFixed(1)}
                </span>
              </div>
              ...
              <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold" style={{ color: getSentimentColor(sentiment.overall) }}>
