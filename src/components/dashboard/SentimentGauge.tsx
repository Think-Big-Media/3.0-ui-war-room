...
const GaugeChart = memo<{ value: number; label: string }>(
  ({ value, label }) => {
    ...
    useEffect(() => {
      ...
      GAUGE_CONFIG.zones.forEach((zone) => {
        const startAngle =
          minAngle +
          ((zone.min - minValue) / (maxValue - minValue)) *
            (maxAngle - minAngle);
        const endAngle =
          minAngle +
          ((zone.max - minValue) / (maxValue - minValue)) *
            (maxAngle - minAngle);
...
const TrendChart = memo<{ data: Array<{ timestamp: Date; score: number }> }>(
  ({ data }) => {
    if (data.length === 0) {
      return (
        ...
        <svg className="w-full h-full">
          <defs>
            ...
            >
              <stop
                offset="0%"
                stopColor={getSentimentColor(maxScore)}
                stopOpacity="0.3"
              />
              <stop
                offset="100%"
                stopColor={getSentimentColor(minScore)}
                stopOpacity="0"
              />
            </linearGradient>
          </defs>
