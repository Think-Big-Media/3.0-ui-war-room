    ...
    >
      <div className="space-y-3">
        ...
        >
          ...
          <div className={`space-y-3 transition-all duration-300 ${isPaused ? '' : 'animate-scroll-feed'}`}>
            ...
            {[...feedItems, ...feedItems].map((item, index) => (
              <div
                ...
                className={`bg-black/20 p-3 rounded border-l-2 text-sm min-h-[60px] flex items-center transition-all duration-300 ${getBorderColor(item.type)} ${
                  highlightedFeedId === item.id ? 'bg-white/20 scale-105 shadow-lg z-10 relative' : ''
                }`}
              >
