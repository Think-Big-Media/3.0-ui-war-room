...
const SWOTRadar: React.FC<SWOTRadarProps> = ({ className = '', onBlobClick }) => {
  ...
  const getBlobColor = (type: RadarBlob['type']) => {
    switch (type) {
      case 'strength': return 'from-green-500/80 to-transparent';
      case 'weakness': return 'from-red-500/80 to-transparent';
      case 'opportunity': return 'from-blue-500/80 to-transparent';
      case 'threat': return 'from-orange-500/80 to-transparent';
    }
  };
        ...
        <div className="relative w-full h-[400px] bg-gradient-radial from-emerald-500/5 to-transparent border-2 border-emerald-500/20 rounded-full flex items-center justify-center">
          ...
          {radarBlobs.map((blob) => (
            ...
            >
              ...
              />
              <div className={`absolute -top-9 left-1/2 transform -translate-x-1/2 bg-black/95 text-white text-xs px-2 py-1 rounded border border-white/20 whitespace-nowrap transition-opacity z-10 ${
