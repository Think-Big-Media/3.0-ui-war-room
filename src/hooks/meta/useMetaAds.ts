...
import { MetaAdService, type AdCreateParams, type AdUpdateParams, type AdListParams, type AdInsightsParams } from '@/api/meta/ads';
...
export const adKeys = {
  ...
  preview: (adId: string, format?: string) =>
    [...adKeys.detail(adId), 'preview', format] as const,
 ...
 */
export function useMetaAdCreative(
  creativeId: string,
  options?: UseQueryOptions<Creative>,
) {
  const { adService } = useMetaClient();
...
export function useCreateMetaAd(
  ...
  return useMutation({
    mutationFn: ({ accountId, params }) =>
      adService.createAd(accountId, params),
...
export function useUpdateMetaAd(
  ...
  return useMutation({
    mutationFn: ({ adId, params }) =>
      adService.updateAd(adId, params),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: adKeys.lists() });
 ...
 */
export function useDeleteMetaAd(
 ...
 * Hook to pause an ad
 */
export function usePauseMetaAd(
  options?: UseMutationOptions<Ad, Error, string>,
) {
  const queryClient = useQueryClient();
  const { adService } = useMetaClient();
 ...
 */
export function useResumeMetaAd(
  options?: UseMutationOptions<Ad, Error, string>,
) {
  const queryClient = useQueryClient();
  const { adService } = useMetaClient();
...
export function useDuplicateMetaAd(
  options?: UseMutationOptions<Ad, Error, {
    adId: string;
    newName: string;
...
export function useBatchCreateMetaAds(
  ...
  return useMutation({
    mutationFn: ({ accountId, ads }) =>
      adService.batchCreateAds(accountId, ads),
    onSuccess: (results) => {
      queryClient.invalidateQueries({ queryKey: adKeys.lists() });
...
export function useCreateMetaAdCreative(
  options?: UseMutationOptions<Creative, Error, {
    accountId: string;
  ...
  return useMutation({
    mutationFn: ({ accountId, params }) =>
      adService.createAdCreative(accountId, params),
    onSuccess: (data) => {
